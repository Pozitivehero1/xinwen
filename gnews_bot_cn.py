...
def summarize_with_mistral(title: str, text: str) -> tuple[str, str]:
    """Возвращает (русский_заголовок, русский_пересказ) объёмом ≈900 символов, без источников и домыслов."""
    if not text or len(text) < 50:
        return title, text

    prompt = f"""Твоя задача — пересказать новость, используя ТОЛЬКО информацию из раздела "ТЕКСТ НОВОСТИ". Твои знания устарели, не добавляй ничего от себя.

**ПРАВИЛА:**
1. НЕ упоминай источники (BBC, журналист, сайт, источник, по информации).
2. НЕ придумывай имена, счета, даты, которых нет в тексте.
3. Пиши от первого лица (как будто ты сам сообщаешь новость).
4. Пересказ должен быть подробным, но без воды. Используй почти весь лимит Telegram для подписи к фото: заголовок + пересказ ≈ 950-1000 символов.
5. Заголовок — краткий (до 80 символов), пересказ — 4–7 предложений, включая ключевые детали, цифры, имена.
6. Не начинай со слов "Новость:", "Заголовок:".
7. Выдай ответ в формате:
    ...
    try:
        resp = requests.post(
            ...
            json={
                ...
                "temperature": 0.3,
                "max_tokens": 700          # увеличен для подробного пересказа
            },
            timeout=20
        )
        if resp.status_code == 200:
            ...
            for phrase in [r'по информации\s*\w*', r'согласно\s*\w*', r'журналист\s*\w*',
                           ...
                           r'bbc', r'sport', r'ru', r'com', r'сообщает']:
